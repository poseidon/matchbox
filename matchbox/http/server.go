...
func (s *Server) HTTPHandler() http.Handler {
	...
	mux.Handle("/boot.gpxe", chain(gpxeInspect()))
	mux.Handle("/boot.gpxe.0", chain(gpxeInspect()))
	mux.Handle("/gpxe", chain(s.selectProfile(s.core, s.gpxeHandler())))
	// Ignition Config
	...
	if s.signer != nil {
		...
		mux.Handle("/boot.gpxe.sig", signerChain(gpxeInspect()))
		mux.Handle("/boot.gpxe.0.sig", signerChain(gpxeInspect()))
		mux.Handle("/gpxe.sig", signerChain(s.selectProfile(s.core, s.gpxeHandler())))
	...
	if s.armoredSigner != nil {
		...
		mux.Handle("/boot.gpxe.asc", signerChain(gpxeInspect()))
		mux.Handle("/boot.gpxe.0.asc", signerChain(gpxeInspect()))
		mux.Handle("/gpxe.asc", signerChain(s.selectProfile(s.core, s.gpxeHandler())))
